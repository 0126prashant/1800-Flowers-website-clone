          ...
          <Card maxW='250px' key={ele.id}>
            <CardBody>
              ...
              <Stack mt='6' spacing='3'>
                ...
                <Text>
                  "Sending you these flowers to brighten up your day and let you know that I'm thinking of you."
                </Text>
              </Stack>
            </CardBody>
            <Divider />
            <CardFooter>
              ...
              <ButtonGroup size="sm" ml="2">
                <Button onClick={() => handleQuantityChange(ele.id, 'decrement')}>
                  -
                </Button>
                <Button>
                  {ele.quantity}
          ...
          {filteredCartItems.map((item,ind) => (
            <Box key={item.id} mb="4">
              <Box display="flex" justifyContent="space-between" alignItems="center">
                ...
                <Text fontWeight="bold">Price: {ind+1}  </Text>
                <Text fontSize="sm">Rs : {item.price}</Text>
              </Box>
          ))}
          <Box mb="4">
            <Text fontWeight="bold">Total:</Text>
            <Text fontSize="sm">Rs {totalPrice.toFixed(2)}</Text>
          </Box>
          <Box display="flex" justifyContent="space-between" alignItems="center">
